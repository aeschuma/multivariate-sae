...
parameters {
    ...
    real<lower=0> sigma_theta; // standard deviation of the shared IID REs on region
...
transformed parameters {
    ...
    for (i in 1:N) {
         mu[i, 1] = alpha1[regions[i]] + (theta[regions[i]] * delta); //
         mu[i, 2] = alpha2[regions[i]] + (theta[regions[i]] / delta); //
    }
...
model {
    ...
    sigma_theta ~ student_t(3,0,1); // leads to a half t prior
