...
while(any_zeros_ones) {
    tmpdat <- dat[sample(1:nrow(dat), ceiling(samplesize*nrow(dat))),]
    any_zeros_ones <- any(table(tmpdat$strata) %in% c(0, 1, 2))
}
dat <- tmpdat
...
write_rds(stage_1_list, file = paste0("../../../Dropbox/dissertation_2/survey-csmf/results/ken2014-hazwaz/ken2014-subsample-",gsub("\\.","_",samplesize),"-hazwaz-stage-1.rds"))
