...
for (i in 1:length(stage_2_list)) {
    ...
    res.tmp <- tibble(model.name = rep(tmpmodname, n_regions),
                      ...
                      admin1.name = results$admin1.name,
...
## save table
write_rds(modcomp, "../../../Dropbox/dissertation_2/survey-csmf/results/proj-2-chapter-results/waic-dic-cpo-table.rds")

kable(modcomp, col.names = c("Model","WAIC", "DIC", "CPO"),
      format = "markdown", escape = FALSE)

# Posterior median comparison plots ####
