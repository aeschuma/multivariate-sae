...
write_rds(res_list, 
          file = "../../../Dropbox/dissertation_2/survey-csmf/results/ken2014-hazwaz/ken2014-hazwaz-stage-2-bivariate-shared-coreg.rds")
...
res.re <- summary(mod.stan.bi.shared, pars = re_params, probs = c(0.1, 0.5, 0.9))$summary
...
res.pred <- summary(mod.stan.bi.shared, pars = "preds", probs = c(0.1, 0.5, 0.9))$summary[, c("10%", "50%", "90%")]
...
res.df.bi.shared <- as.data.frame(cbind(results$admin1.name,
...
names(res.df.bi.shared) <- c("admin1.name", 
                             "haz.iid.50", "waz.iid.50",
                             "haz.icar.50", "waz.icar.50",
                             "haz.pred.50", "haz.pred.width80",
                             "waz.pred.50", "waz.pred.width80")
rownames(res.df.bi.shared) <- NULL

for (vv in 1:ncol(res.df.bi.shared)) {
    if (names(res.df.bi.shared)[vv] == "admin1.name") next
    res.df.bi.shared[, vv] <- as.numeric(res.df.bi.shared[, vv])
}


## ----bi-shared-plots, fig.height = 6, fig.width = 6, cache = FALSE---------------------------
n_cats <- 15
...
outcomes.names <- c("HAZ IID RE median", "WAZ IID RE median",
                    "HAZ ICAR RE median", "Shared ICAR RE median")

plot_maps(poly = poly.adm1, outcome.df = res.df.bi.shared, 
...
## ----bi-shared-plots-2, fig.height = 6, fig.width = 6, cache = FALSE-------------------------
# plot predictions
